...
class TaskViewHolder(itemView: View, val listener: TaskListener) :
    ...
    fun bindData(task: TaskModel) {
        ...
        // Get priority description by task.id
        this.mTextPriority.text = mPriorityRepository.getDescriptionById(task.priorityId)

        // Deformat and set the correct Date
        val deformatedDate = SimpleDateFormat("yyyyy-MM-dd").parse(task.dueDate)
        this.mTextDueDate.text = mDateFormat.format(deformatedDate)
